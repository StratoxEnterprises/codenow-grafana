...
role_attribute_path = contains(cnAccounts[?name=='demo-tenant'].role, 'owner') && 'Owner' || contains(cnAccounts[?name=='demo-tenant'].role, 'User') || 'None' 

regex_org_role_mapper = \"\"\"(^(?P<org>\\w+)#admin):Editor (^(?P<org>\\w+)#viewer):Viewer\"\"\"
...
```
Since user is user in `demo-tenant` account, based on `cnEnvironments` he gets `Viewer` grafana role to `rru` grafana organization and `Editor` grafana role to `omi-env1` grafan organization.  
